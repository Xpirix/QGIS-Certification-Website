...
class Project(models.Model):
    ...
    certificate_credit = models.IntegerField(
        help_text=_("Cost to issue a certificate, i.e. a certificate cost 1 credit"),
        default=1,
        null=True,
        blank=True,
    )
    ...
    template_certifying_organisation_certificate = models.ImageField(
        help_text=_(
            "Background template of the certificate for certifying "
            "organisation. "
            "Most browsers support dragging the image directly on to "
            'the "Choose File" button above.'
        ),
        upload_to=os.path.join(MEDIA_ROOT, "images/projects/organisation_certificates"),
        blank=True,
    )

    is_certification = models.BooleanField(
        help_text=_("Whether this Project has Certification."), default=True
    )
